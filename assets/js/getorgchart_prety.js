getOrgChart = function(b, a) {
    ...
    this.element = b;
    this.render = (this.config.render == "AUTO") ? getOrgChart._R() : this.config.render;
    this._ai = [];
    this._ak = [];
    this._a1 = [];
    this._zz = 0;
...
getOrgChart.prototype._as = function() {
    this._zS = get._f().msie ? this.element.clientWidth : window.getComputedStyle(this.element, null).width;
    this._zS = parseInt(this._zS);
    if (this._zS < 3) {
        this._zS = 1024;
        this.element.style.width = "1024px"
    }
    this._zW = get._f().msie ? this.element.clientHeight : window.getComputedStyle(this.element, null).height;
    ...
    var a = getOrgChart.INNER_HTML.replace("[theme]", this.config.theme).replace("[color]", this.config.color).replace(/\[height]/g, this._aG).replace(/\[toolbar-height]/g, this.theme.toolbarHeight);
    if (getOrgChart._zA) {
        a = a.slice(0, -6);
        a += getOrgChart._zA
    }
...
getOrgChart._A.prototype._aI = function() {
    ...
    for (r = 0; r < e.length; r++) {
        if (e[r].className == "get-link-label") {
            this._au.push(e[r])
        } else {
            this._zi.push(e[r])
        }
...
getOrgChart._zX = function(v, o, g) {
    ...
    for (var d = v.config.maxDepth - g; a != null && b != null && c <= d;) {
        ...
        var u = (b._aO + m + v._1(b) + v.config.subtreeSeparation) - (a._aO + n);
        if (u > 0) {
            var s = o;
            var p = 0;
...
getOrgChart._zf = function(j, d, b, k, m) {
    if (b <= j.config.maxDepth) {
        ...
        if (d._T() != 0) {
            getOrgChart._zf(j, d._M(), b + 1, k + d._ao, m + c + j.config.levelSeparation)
        }
        var f = d._2();
        if (f != null) {
...
getOrgChart._zp = function(a) {
    for (i = 0; i < a._zi.length; i++) {
        ...
        while (b > c) {
            a._zi[i].textContent = a._zi[i].textContent.substring(0, a._zi[i].textContent.length - 4);
            a._zi[i].textContent += "...";
...
getOrgChart.person.prototype.compareTo = function(b) {
    var a = this;
    if (a === undefined || b === undefined || a._zE === undefined || a._zD === undefined || b._zE === undefined || b._zD === undefined) {
        return false
    } else {
        ...
        switch (f.render) {
            case "SVG":
                switch (f.config.linkType) {
                    case "M":
                        e.push('<path class="link"   d="M' + g + "," + l + " " + h + "," + m + " " + i + "," + n + " L" + j + "," + o + '"/>');
                        break;
                    case "B":
                        e.push('<path class="link"  d="M' + g + "," + l + " C" + h + "," + m + " " + i + "," + n + " " + j + "," + o + '"/>');
                        break
                }
                var c = f._P(d);
                if (c != null) {
                    e.push('<text width="200" data-position="' + b + "," + this._L() + '" " class="get-link-label" x="' + j + '" y="' + (o + a) + '">' + c.text + "</text>")
                }
                break;
            case "VML":
                break
...
get._s = function(a, c, b, h, j, d) {
    ...
    function f() {
        for (var s in b) {
            var t = getOrgChart.util._e(["top", "left", "right", "bottom"], s.toLowerCase()) ? "px" : "";
...
getOrgChart.prototype._zh = function(c) {
    ...
    if (e) {
        this._A._u.innerHTML = getOrgChart.DETAILS_VIEW_ID_IMAGE.replace("[src]", e)
    } else {
...
getOrgChart.prototype._ap = function(f, d) {
    ...
    if (this._aT.dragStart) {
        ...
        var e = g[3] / this._aG;
        var c = h > e ? h : e;
        g[0] = -((this._aT.mouseLastX - this._aT.dragStart.x) * c) + this._aT.dragStart.viewBoxLeft;
        g[1] = -((this._aT.mouseLastY - this._aT.dragStart.y) * c) + this._aT.dragStart.viewBoxTop;
        g[0] = parseInt(g[0]);
        g[1] = parseInt(g[1]);
...
getOrgChart.prototype._o = function() {
    if ((this._ze.showIndex < this._ze.found.length - 1) && (this._ze.found.length != 0)) {
...
getOrgChart.prototype.updatePerson = function(b, c, a) {
    ...
    if (b == "") {
        ...
        for (t = this._aN.length - 1; t >= 0; t--) {
            if (this._aN[t].id == b) {
                if (this._aN[t].pid == null || typeof(this._aN[t].pid) == "undefined" || this._aN[t].pid == "") {
                    this._aN[t].data = a
                } else {
                    if (this._aN[t].pid == c) {
                        this._aN[t].data = a
